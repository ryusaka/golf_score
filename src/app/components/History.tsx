    ...
    return (
      <>
            ...
            {logs.map((h, idx) =>
              <ExpansionPanel key={moment(h.date).toString()} className={classes.expansion}>
                ...
                <ExpansionPanelActions>
                  <Button onClick={() => this.setState({remove: idx})} size='small'>削除</Button>
                </ExpansionPanelActions>
              </ExpansionPanel>
            )}
        ...
        <Dialog open={remove !== null} onClose={() => this.setState({remove: null})}>
          {remove !== null &&
            <DialogContent>
              <div className={classes.dialogFieldName}>{logs[remove].field.name}({moment(logs[remove].date).format('YYYY-MM-DD')})</div>
              <div>のスコアを削除しますか？</div>
            </DialogContent>
          }
          <DialogActions>
            <Button onClick={() => this.setState({remove: null})}>キャンセル</Button>
            <Button variant='contained' onClick={this.remove}>削除する</Button>
          </DialogActions>
        </Dialog>
      </>
    )
