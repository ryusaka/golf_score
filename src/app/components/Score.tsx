      ...
      <div className={classes.root}>
        <Header>
          <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'center' }}>
            ...
            <h3>
              HOLE: <span style={{ fontSize: 24, marginRight: 10 }}>{now}</span> PAR:{' '}
              <span style={{ fontSize: 24 }}>{field.holes[now - 1].par}</span>
            </h3>
          </div>
        </Header>
        ...
        <Paper className={classes.bottom}>
          ...
          <div style={{ width: 10 }} />
          {now === 9 ? (
            <Button className={classes.button} color='secondary' variant='contained' onClick={this.finish}>
              終了
            </Button>
          ) : (
            <Button className={classes.button} color='primary' variant='contained' onClick={() => this.props.forward()}>
              <ForwardIcon />
              {now + 1}
            </Button>
          )}
